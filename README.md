...
**서버 수정**(서버 검사 화면 또는 대시보드)에서 SSH·DB 접속 정보를 저장해 두면,
**검사** 실행 시 자동으로 서버에 접속해 내부까지 점검합니다.

- **입력**: SSH(포트/ID/Password), DB(포트/ID/Password) — 필요한 쪽만 입력해도 됩니다
- **비밀번호는 Fernet으로 암호화해 저장**합니다. 키는 `.env`의 `CRED_KEY`이며 커밋되지 않습니다
- 저장된 비밀번호는 **화면·API로 다시 내려보내지 않습니다**. 수정 시 비워두면 기존 값이 유지됩니다
